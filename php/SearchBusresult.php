...
<div id="searchrow">
		...
		<button type="submit" class="srch" id="btnsearch" name="search">Search Bus</button>
...
$sql = "select * from busreservation WHERE BSPoint='".$from."' AND BEPoint ='".$to."' AND BDate ='".$date."' " ;
...
if($result->num_rows>0){
  ...
  while($row = $result->fetch_assoc()){
    ...
    <div class='Result'>";
    echo "<p></p>";
    echo "<p>Bus Reg Number - $dbreg</p>";
    echo "<p>Route Number - $dbroute</p>";

    echo "<p>From - $dbbspoint($dbtime)</p>";
    echo "<p>To - $dbbepoint</p>";
    echo "<p>$dbdate</p>";
    echo"<h2> Ticket Price</h2>";
    echo"<h2> $dbprice</h2>";
    echo "<a href='../php/seat.php?ID=$dbID'><button class='btnBook' name='submit'>Book</button></a>";
    echo"</div>";
